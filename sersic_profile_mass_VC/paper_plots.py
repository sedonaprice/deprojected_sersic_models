...
def plot_compare_mencl(fileout=None, output_path=None, table_path=None, q_arr=[1., 0.4, 0.2]):
    ...
    nstep = 1
    nrange = nmax-nmin
    n_arr = np.linspace(nmin,nmax, num=(nrange)/nstep+1)
...
def plot_rhalf_3D_sersic_potential(output_path=None, table_path=None, fileout=None):
    ...
    nstep = 0.1 
    nrange = nmax-nmin
    n_arr_plotq = np.linspace(nmin,nmax, num=(nmax-nmin)/nstep+1)
    color_arr_plotq = []
    ls_arr_plotq = []
    labels_plotq = []
...
def plot_virial_coeff(fileout=None, output_path=None, table_path=None, 
    ...
    try:
        ...
        nmax = 8.
        nstep = 0.1  
        n_arr = np.linspace(nmin,nmax, num=(nmax-nmin)/nstep+1)
    ...
    ann_arr = [ r'$$k_{\mathrm{tot}}(R_E) = \frac{M_{\mathrm{tot}} G }{v_c(R_E)^2 R_E}$$',
                r'$$k_{\mathrm{3D}}(R_E) = \frac{M_{\mathrm{encl,3D}}(<R_E) G }{v_c(R_E)^2 R_E}$$' ]
    ...
    for i in xrange(n_rows):
        for j in xrange(n_cols):
            ...
            if ax2 is not None:
                ...
                if (j == n_cols-1):
                    if ylabels2[k] is not None:
                        ax2.set_ylabel(ylabels2[k], fontsize=fontsize_labels_sm, color=color_fdm)
                    if (i < n_rows-1):
                    ...
                    ax2.tick_params(axis='y', direction='in', color=color_fdm, 
                            labelsize=fontsize_ticks_sm, colors=color_fdm)
                else:
                    ax2.yaxis.set_major_locator(MultipleLocator(0.25))
                    ax2.tick_params(labelright='off')
                ax2.tick_params(axis='y', direction='in', color=color_fdm, which='both')
            ########
            if logradius:
    ...
    for i in xrange(n_cols):
        ...
        for k, n in enumerate(n_arr):
            for j, q in enumerate(q_arr):
                ...
                r_arr = ks_dict[types[i]]['n={}'.format(n)]['rarr']
...
def make_all_paper_plots(output_path=None, table_path=None):
    ...
    # Figure 6
    plot_fdm_calibration(Mbaryon_arr=[1.e10, 10**10.5, 1.e11], 
                q_disk_arr = [0.01, 0.05, 0.1, 0.2, 0.25, 0.4, 0.6, 0.8, 1.], 
